...
from nlp_analysis import extrair_palavras_chave, treinar_modelo_ideal, prever_configuracao_ideal, extrair_atributos
...
class MainWindow(QMainWindow):
    ...
    def prever_entropia(self):
        ...
        atributos = extrair_atributos(descricao_tarefa)
        print("Atributos extraídos:", atributos)

        if modelo:
            entropia_predita = prever_entropia(modelo, descricao_tarefa, tempo_estimado, complexidade)
            entropia_maxima = 3
            entropia_normalizada = entropia_predita / entropia_maxima
            porcentagem_aproximacao = (1 - entropia_normalizada) * 100
            ...
            tempo_ideal, complexidade_ideal = prever_configuracao_ideal(modelo_ideal, atributos)
            sugestao = f"Com base nos atributos extraídos, a configuração ideal sugerida para esta tarefa é:\n" \
                   f"Tempo: {tempo_ideal:.2f} horas\n" \
                   f"Complexidade: {complexidade_ideal:.2f}"
            self.text_resultado.setPlainText(resultado + "\n\n" + sugestao)
        else:
...
if __name__ == "__main__":
    ...
    modelo_ideal = treinar_modelo_ideal(caminho_csv_transformado)
